...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &     )
      ...
      real(kind=8) cff
      integer(kind=4) i,ierr, lstr,lvar,lenstr, nf_fread, ifrc, ncid
 ...
 100  ncid=ncidbulk(ifrc)
     ...
     &                                  frcname(1:lstr)
            ...
            if (mynode.eq.0) write(*,1) 'iron_time', frcname(1:lstr)
  ...
  1   format(/1x,'WARNING: get_iron cannot find variable ''',
     &     A, ''''/7x, 'in forcing netCDF file ''', A, '''.')
      do while (iron_time(itiron).lt.time+0.5_8*dt .and. 
     &                         ierr.eq.nf_noerr)
        call advance_cycle (iron_cycle, ntiron, iron_ncycle,
     &                                 iron_rec,     ierr)
        if (ierr .eq. nf_noerr) then
          ierr=nf_get_var1_double (ncid, iron_tid, iron_rec, cff)
          if (ierr .eq. nf_noerr) then
            itiron=3-itiron
            iron_time(itiron)=cff*day2sec + iron_cycle*iron_ncycle
            ierr=nf_fread (irong(-1,-1,itiron), ncid,
  ...
  2   format(/1x,'ERROR in get_iron: cannot read variable ''', A,
     &                                    ''' for record ', I4/)
      ...
      implicit none
      integer(kind=4) Istr,Iend,Jstr,Jend, i,j, it1,it2,ierr
      real(kind=8) cff, cff1,cff2, cff3,cff4, val1
      integer(kind=4), parameter ::
     &               LLm=435, MMm=660, N=60
      integer(kind=4), parameter ::
     &      NP_XI=8, NP_ETA=32, NSUB_X=1, NSUB_E=1
...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &                 , g=9.81_8
      ...
      if ((istr.eq.iwest .and. jstr.eq.jsouth)  .and. ( cff1.lt.dt )) 
     &                        synchro_flag=.true.
      if (cff1.ge.0._8 .and. cff2.ge.0._8 ) then
        cff=1._8/(cff1+cff2)
        cff1=cff1*cff
        cff2=cff2*cff
        do j=jstrR,jendR
