     ...
     &     )
      ...
      integer(kind=4) ierr,  record, i, lstr, lvar, lenstr
...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &
      ...
      if (wrthis(indxZ)) then
        ierr=nf_fwrite (zeta(-1,-1,knew), ncidhis, hisZ,
     &                                              record, r2dvar)
        if (ierr .ne. nf_noerr) then
      ...
      if (wrthis(indxUb)) then
        ierr=nf_fwrite (ubar(-1,-1,knew), ncidhis, hisUb,
     &                                              record, u2dvar)
        if (ierr .ne. nf_noerr) then
      ...
      if (wrthis(indxVb)) then
        ierr=nf_fwrite (vbar(-1,-1,knew), ncidhis, hisVb,
     &                                            record, v2dvar)
        if (ierr .ne. nf_noerr) then
      ...
      if (wrthis(indxU)) then
        ierr=nf_fwrite (u(-1,-1,1,nstp), ncidhis,
     &                             hisU,  record,  u3dvar)
        if (ierr .ne. nf_noerr) then
      ...
      if (wrthis(indxV)) then
        ierr=nf_fwrite (v(-1,-1,1,nstp), ncidhis,
     &                             hisV,  record,  v3dvar)
        if (ierr .ne. nf_noerr) then
      ...
      do itrc=1,NT
        if (wrthis(indxT+itrc-1)) then
          ierr=nf_fwrite (t(-1,-1,1,nstp,itrc), ncidhis,
     ...
     &                 record
      ...
      do itrc=1,NT_sed
        if (wrthis(indxSedFirst+itrc-1)) then
          ierr=nf_fwrite(t_sed(-1,-1,itrc), ncidhis,
     ...
     &                      record, ierr, nf_strerror(ierr)
      ...
      if (wrthis(indxAkt)) then
        ierr=nf_fwrite (Akt(-1,-1,0,itemp), ncidhis,
     &                                hisAkt, record, w3dvar)
      ...
      if (wrthis(indxAks)) then
        ierr=nf_fwrite (Akt(-1,-1,0,isalt), ncidhis,
     &                                hisAks, record, w3dvar)
      ...
      if (wrthis(indxHbls)) then
        ierr=nf_fwrite (hbls(-1,-1,nstp), ncidhis,
     &                             hisHbls, record, r2dvar)
      ...
      if (wrthis(indxHbbl)) then
        ierr=nf_fwrite (hbbls(-1,-1,nstp), ncidhis,
     &                              hisHbbl, record, r2dvar)
      ...
      ierr=nf_fwrite (pH_hist(-1,-1), ncidhis,
     &                              hisPH, record, r2dvar)
      if (ierr .ne. nf_noerr) then
     ...
     &                  nf_strerror(ierr)
      ...
      ierr=nf_fwrite (pCO2sw(-1,-1), ncidhis,
     ...
     &                  nf_strerror(ierr)
      ...
      ierr=nf_fwrite (pCO2air(-1,-1), ncidhis,
     ...
     &                  nf_strerror(ierr)
      ...
      ierr=nf_fwrite (PARinc(-1,-1), ncidhis,
     ...
     &                  nf_strerror(ierr)
      ...
      ierr=nf_fwrite (PAR(-1,-1,1), ncidhis,
