...
C$OMP THREADPRIVATE(/prv_scrch/)
     ...
     &     + 3
      ...
      real(kind=8), dimension(istr-2:iend+2,jstr-2:jend+2,N) :: 
     &                           ru,rv,Hz_bak,
     &                                                      Hz_fwd
      real(kind=8), dimension(istr-2:iend+2,0:N) :: CF,FC, WC,DC
      real(kind=8), dimension(istr-2:iend+2,jstr-2:jend+2)  :: 
     &                          UFx,UFe,VFx,VFe,
     &                                                    wrk1,wrk2
      real(kind=8) dtau, cf_stp,cf_bak,  cff, FlxDiv, stress
...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &                                                           )
        ...
        do itrc=1,NT
          ...
          do j=jstr,jend
            do i=imin,imax+1
              UFx(i,j)=(t(i,j,k,nrhs,itrc)-t(i-1,j,k,nrhs,itrc))
     &                                               *umask(i,j)
            enddo
          enddo
          if (istr.eq.iwest .and. .not.west_exchng) then
            do j=jstr,jend
              UFx(istr-1,j)=UFx(istr,j)
            enddo
          endif
          if (iend.eq.ieast .and. .not.east_exchng) then
            do j=jstr,jend
              UFx(iend+2,j)=UFx(iend+1,j)
            enddo
          endif
          do j=jstr,jend
            do i=istr-1,iend+1
              wrk1(i,j)=0.5_8*(UFx(i+1,j)+UFx(i,j))
            enddo
          enddo
          do j=jstr,jend
            do i=istr,iend+1
              UFx(i,j)=0.5_8*( t(i,j,k,nrhs,itrc)+t(i-1,j,k,nrhs,itrc)
     &                  -0.3333333333333333_8*( wrk1(i,j)-wrk1(i-1,j))
     &                                                 )*FlxU(i,j,k)
            enddo
          enddo
          if (jstr.eq.jsouth .and. .not.south_exchng) then
          ...
          do j=jmin,jmax+1
            do i=istr,iend
              VFe(i,j)=(t(i,j,k,nrhs,itrc)-t(i,j-1,k,nrhs,itrc))
     &                                               *vmask(i,j)
            enddo
          enddo
          if (jstr.eq.jsouth .and. .not.south_exchng) then
            do i=istr,iend
              VFe(i,jstr-1)=VFe(i,jstr)
            enddo
          endif
          if (jend.eq.jnorth .and. .not.north_exchng) then
            do i=istr,iend
              VFe(i,jend+2)=VFe(i,jend+1)
            enddo
          endif
          do j=jstr-1,jend+1
            do i=istr,iend
              wrk1(i,j)=0.5_8*(VFe(i,j+1)+VFe(i,j))
            enddo
          enddo
          do j=jstr,jend+1
            do i=istr,iend
              VFe(i,j)=0.5_8*( t(i,j,k,nrhs,itrc)+t(i,j-1,k,nrhs,itrc)
     &                   -0.3333333333333333_8*(wrk1(i,j)-wrk1(i,j-1))
     &                                                 )*FlxV(i,j,k)
            enddo
          enddo
          do j=jstr,jend
     ...
     &                    -dtau*pm(i,j)*pn(i,j)*( UFx(i+1,j)-UFx(i,j)
     &                                           +VFe(i,j+1)-VFe(i,j) )
     ...
     &                                                             )
        ...
        do j=jstr,jend
          do i=istrU-1,iend
            UFx(i,j)=0.25_8*( u(i,j,k,nrhs)+u(i+1,j,k,nrhs)
     ...
     &                      -delta*(wrk2(i,j)+wrk2(i+1,j)))
        ...
        do j=jstrV-1,jend
          do i=istr,iend
            VFe(i,j)=0.25_8*( v(i,j,k,nrhs)+v(i,j+1,k,nrhs)
     &                        -delta*(wrk1(i,j)+wrk1(i,j+1))
     &                   )*( FlxV(i,j,k)+FlxV(i,j+1,k)
     &                      -delta*(wrk2(i,j)+wrk2(i,j+1)))
          enddo
        enddo
        if (jstr.eq.jsouth .and. .not.south_exchng) then
          jmin=jstr
        ...
        do j=jstr,jend+1
          do i=istrU,iend
            UFe(i,j)=0.25_8*( u(i,j,k,nrhs)+u(i,j-1,k,nrhs)
     &                        -delta*(wrk1(i,j)+wrk1(i,j-1))
     ...
     &                     -delta*(wrk2(i,j)+wrk2(i-1,j)))
        ...
        do j=jstrV,jend
          do i=istr,iend+1
            VFx(i,j)=0.25_8*( v(i,j,k,nrhs)+v(i-1,j,k,nrhs)
     &                        -delta*(wrk1(i,j)+wrk1(i-1,j))
     ...
     &                                                 ))
              wrk1(i,j)=rdrg + cff*(vonKar/log(1._8+0.5_8*Hz(i,j,1)
     &                             /Zob))**2
            enddo
          enddo
        else
          do j=jstrV-1,jend
            do i=istrU-1,iend
              wrk1(i,j)=rdrg
              wrk1(i,j)=min(wrk1(i,j), 0.8_8*Hz(i,j,1)/dt)
            enddo
          enddo
        endif
        call ext_copy_prv2shr_2d_tile (istr,iend,jstr,jend, wrk1,r_D)
      do j=jstr,jend
        do itrc=1,NT
          do k=1,N-1
            do i=istr,iend
     ...
     &            -dtau*pm(i,j)*pn(i,j)*(FC(i,k)-FC(i,k-1))
          ...
          iAkt=min(itrc,isalt)
          do i=istr,iend
            FC(i,1)=2._8*dtau*Akt(i,j,1,iAkt)/( Hz_fwd(i,j,2)
     &                                       +Hz_fwd(i,j,1))
            DC(i,0)=dtau*pm(i,j)*pn(i,j)
            WC(i,1)=DC(i,0)*Wi(i,j,1)
            cff=1._8/(Hz_fwd(i,j,1) +FC(i,1)+max(WC(i,1),0._8))
     ...
     &                                         +Hz_fwd(i,j,k) )
              ...
              cff=1._8/( Hz_fwd(i,j,k) +FC(i,k)+max(WC(i,k),0._8)
     &                             +FC(i,k-1)-min(WC(i,k-1),0._8)
     &                  -CF(i,k-1)*(FC(i,k-1)+max(WC(i,k-1),0._8))
     &                                                          )
              CF(i,k)=cff*(FC(i,k)-min(WC(i,k),0._8))
     ...
     &                                   
     &                +0.5_8*dtau*(wrk1(i,j)+wrk1(i-1,j))
     ...
     &                                                        )
              CF(i,k-1)=cff*( FC(i,k-1)+max(WC(i,k-1),0._8) )
              DC(i,k)=cff*(DC(i,k)+DC(i,k+1)*(FC(i,k)-min(WC(i,k),
     &                              0._8)))
            enddo
          enddo
          do i=istr,iend
     ...
     &                                   
     &                +0.5_8*dtau*(wrk1(i,j)+wrk1(i,j-1))
     &                                       +FC(i,1)+max(WC(i,1),0._8)
     &                              -CF(i,1)*(FC(i,1)-min(WC(i,1),0._8))
     &                                                               )
          enddo
