...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &     ierr)
      ...
      nrec_phys_flux_his=max(nrec_phys_flux_his,1)
     ...
     &        nf_strerror(ierr)
      ...
      do iflux = 1, NT_PFA
        if (wrt_pfa(iflux)) then
         ...
         ierr = nf_fwrite(HorXAdvFlux(-1,-1,1,iflux_out),
     ...
     &           record, ierr, nf_strerror(ierr)
      ...
      do iflux = 1, NT_PFA
        if (wrt_pfa(iflux)) then
         iflux_out = pfa_out(iflux)
         ierr = nf_fwrite(HorYAdvFlux(-1,-1,1,iflux_out),
     ...
     &           record, ierr, nf_strerror(ierr)
      ...
      do iflux = 1, NT_PFA
        if (wrt_pfa(iflux)) then
         iflux_out = pfa_out(iflux)
         ierr = nf_fwrite(VertAdvFlux(-1,-1,0,iflux_out),
