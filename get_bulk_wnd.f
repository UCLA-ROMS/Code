 ...
 100  ncid=ncidbulk(iblk)
     ...
     &                                   wnd_ncycle, wnd_rec, ierr)
          ...
          elseif (iblk .lt. max_blk) then
            if (mynode.eq.0) write(*,1) 'uwnd ',blkname(1:lstr)
            iblk=iblk+1
            goto 100
          else
  ...
  2   format(1x, '###EMFMDS ERROR: get_wnd :: Cannot find variable ''',
     &                                A, ''' in file ''', A, '''.')
        do while (wnd_time(itwnd).lt.time+0.5_8*dt  .and.
     &                                    ierr.eq.nf_noerr)
        call advance_cycle (wnd_cycle, ntwnd, wnd_ncycle,
     &                                    wnd_rec, ierr)
          if (ierr.eq.nf_noerr) then
            ierr=nf_get_var1_double (ncid, wnd_tid, wnd_rec, cff)
            if (ierr .eq. nf_noerr) then
              itwnd=min(3-itwnd,ntwnd)
              wnd_time(itwnd)=wnd_cycle*wnd_ncycle + cff*day2sec
            ierr=
     &      nf_fread (uwndg(-1,-1,itwnd), ncid,
     &      uwnd_id,wnd_rec, r2dvar)
            ierr=ierr +
     &      nf_fread (vwndg(-1,-1,itwnd), ncid,
     &      vwnd_id,wnd_rec,  r2dvar)
              if (ierr .eq. nf_noerr) then
                 if (mynode.eq.0) write(*,'(6x,A,3x,A,F12.4,2(1x,A,
     &                               I6))')
     &          'get_bulk -- read bulk for all fields',
     &          'wnd_time =', cff, 'rec =',rad_rec
