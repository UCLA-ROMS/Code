     ...
     &     iptr_srflx_day_set,  num_srflx_day
      ...
      real(kind=8) get_atm_pco2, tdays, cff
      integer(kind=4), parameter :: MAX_NTIME=840
      real(kind=4) buffer(MAX_NTIME)
      integer(kind=4) ncid, ierr, lstr, lvar, lenstr, pco2_id, pco2_tid
      integer(kind=4) pco2_tdim, ntime, i
      character(len=9), parameter :: pco2_tvar='pco2_time'
      character(len=4), parameter :: pco2_var='pco2'
      integer(kind=4) start(1), count(1)
      lstr=lenstr(pco2_atm_file)
      ierr=nf_open(pco2_atm_file(1:lstr), nf_nowrite, ncid)
      if (ierr .ne. nf_noerr) then
         stop 'could not open pco2 file'
      end if
      ierr=nf_inq_dimid(ncid, pco2_tvar,pco2_tdim)
      if (ierr .ne. nf_noerr) then
         stop 'could not get tdim in pco2 file'
      end if
      ierr=nf_inq_dimlen(ncid, pco2_tdim, ntime)
      if (ierr .ne. nf_noerr) then
         stop 'could not read tdim in pco2 file'
      end if
      lvar=lenstr(pco2_var)
      ierr=nf_inq_varid (ncid, pco2_var(1:lvar), pco2_id)
      if (ierr .ne. nf_noerr) then
         stop 'could not read pco2 var from file'
      end if
      ierr=nf_inq_varid (ncid, 'pco2_time', pco2_tid)
      if (ierr .ne. nf_noerr) then
         stop 'could not read pco2 time from file'
      end if
      start(1) = 1
      count(1) = ntime
      ierr=nf_get_vara_real(ncid,pco2_tid,start,count,buffer)
      if (ierr .ne. nf_noerr) then
         stop 'could not read pco2 time values from file'
      end if
      i = 2
      ...
      if (ierr .ne. nf_noerr) then
         stop 'could not read pco2 values from file'
      end if
