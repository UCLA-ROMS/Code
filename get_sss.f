...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &     )
      ...
      integer(kind=4) ierr, ncid, ifrc, lfrc, lenstr, nf_fread
     $,       iblk
 ...
 100  ncid=ncidbulk(iblk)
     ...
     &              'open netCDF file ''', blkname(1:lfrc), '''.'
        ...
        if (ierr .eq. nf_noerr) then
          ...
          if (ierr .eq. nf_noerr) then
            sss_file_id=iblk
            if (mynode.eq.0) write(*,'(1x,5A,I3)') 'Found ''',sss_var,
     ...
     &                              sss_ncycle, sss_rec, ierr)
              ...
              if (mynode.eq.0) write(*,2) sss_var_time, blkname(1:lfrc)
            endif
          elseif (iblk .lt. max_blk) then
            if (mynode.eq.0) write(*,1) sss_var, blkname(1:lfrc)
            iblk=iblk+1
            goto 100
          else
