...
C$OMP THREADPRIVATE(/prv_scrch/)
     ...
     &     + 3
      ...
      real(kind=8), dimension(istr-2:iend+2,jstr-2:jend+2)     :: FX, 
     &                              FE, wrk1
      real(kind=8) cff
      real(kind=8), parameter :: epsil=1.D-33
      integer(kind=4) itrc, iAkt, iflux
...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &     )
      ...
      real(kind=8) ThisVDF(istr-2:iend+2,jstr-2:jend+2,N)
     ...
     &                                                  *FlxU(i,j,k)
     &          -0.1666666666666666_8*( 
     &                 wrk1(i-1,j)*max(FlxU(i,j,k),0._8)
     &                               +wrk1(i  ,j)*min(FlxU(i,j,k),0._8))
            enddo
          enddo
          if (jstr.eq.jsouth .and. .not.south_exchng) then
            jmin=jstr
     ...
     &                                                  *FlxV(i,j,k)
     &          -0.1666666666666666_8*( 
     &                 wrk1(i,j-1)*max(FlxV(i,j,k),0._8)
     &                               +wrk1(i,j  )*min(FlxV(i,j,k),0._8))
            enddo
          enddo
            do j=jstr,jend
              do i=istr,iend
     ...
     &                           /(Hz(i,j,1)+Hz(i,j,2))
          ...
          do k=2,N-1,+1
            do i=istr,iend
              FC(i,k)=2._8*dt* Akt(i,j,k,iAkt)
     &                            /(Hz(i,j,k)+Hz(i,j,k+1))
              WC(i,k)=Wi(i,j,k)*DC(i,0)
              cff=1._8/( Hz(i,j,k) +FC(i,k)+max(WC(i,k),0._8)
