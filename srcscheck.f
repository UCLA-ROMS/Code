      ...
      integer(kind=4), parameter :: iin=11, iout=12, max_string=128
      character tab*1, backslash*1, string*128
      integer(kind=4) lstr, i, is,ie, indx, iocheck
   ...
   5    if (string(is:is).eq.' ') then
        ...
        if (string(is:is+5).eq.'KRNSRC') then
          sources=.true.
        elseif (string(is:is+4).eq.'IOSRC') then
          sources=.true.
        elseif (string(is:is+3).eq.'SRCS') then
          sources=.true.
        endif
        if (sources) then
