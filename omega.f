...
C$OMP THREADPRIVATE(/prv_scrch/)
     ...
     &     + 3
      integer(kind=4), parameter :: NT=itemp+ntrc_salt+ntrc_pas+ntrc_bio
      real(kind=8) CX(istr-2:iend+2,0:N),  dtau,cdt,
     &    wrk(istr-2:iend+2),      cff, dh,dh2
     ...
     &             +max(FlxV(i,j+1,k),0._8)-min(FlxV(i,j,k),0._8)
        ...
        do k=N-1,1,-1
          do i=istr,iend
            ...
            CX(i,k+1)=CX(i,k+1)-min(Wi(i,j,k),0._8)
            CX(i,k  )=CX(i,k  )+max(Wi(i,j,k),0._8)
          enddo
        enddo
        do k=1,N-1
          do i=istr,iend
