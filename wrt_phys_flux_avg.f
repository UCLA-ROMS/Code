...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &     ierr)
      ...
      nrec_phys_flux_avg=max(nrec_phys_flux_avg,1)
     ...
     &
      ...
      ierr=nf_fwrite (zeta_phys_flux_avg(-1,-1),
     ...
     &
      ...
      do iflux = 1, NT_PFA
        if (wrt_pfa(iflux)) then
         ...
         ierr = nf_fwrite(HorXAdvFlux_avg(-1,-1,1,iflux_out),
     ...
     &           record, ierr, nf_strerror(ierr)
      ...
      do iflux = 1, NT_PFA
        if (wrt_pfa(iflux)) then
         iflux_out = pfa_out(iflux)
         ierr = nf_fwrite(HorYAdvFlux_avg(-1,-1,1,iflux_out),
     ...
     &           record, ierr, nf_strerror(ierr)
      ...
      do iflux = 1, NT_PFA
        if (wrt_pfa(iflux)) then
         iflux_out = pfa_out(iflux)
         ierr = nf_fwrite(VertAdvFlux_avg(-1,-1,
     &        0,iflux_out),
