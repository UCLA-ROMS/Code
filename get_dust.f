...
C$OMP THREADPRIVATE(/priv_scalars/)
 ...
 100  ncid=ncidbulk(ifrc)
     ...
     &                                  frcname(1:lstr)
            ...
            if (mynode.eq.0) write(*,1) 'dust_time', frcname(1:lstr)
  ...
  1   format(/1x,'WARNING: get_dust cannot find variable ''',
     &     A, ''''/7x, 'in forcing netCDF file ''', A, '''.')
      do while (dust_time(itdust).lt.time+0.5_8*dt 
     &                       .and.ierr.eq.nf_noerr)
        call advance_cycle (dust_cycle, ntdust, dust_ncycle,
     &                                 dust_rec,     ierr)
        if (ierr .eq. nf_noerr) then
          ierr=nf_get_var1_double (ncid, dust_tid, dust_rec, cff)
          if (ierr .eq. nf_noerr) then
            itdust=3-itdust
            dust_time(itdust)=cff*day2sec + dust_cycle*dust_ncycle
            ierr=nf_fread (dustg(-1,-1,itdust), ncid,
