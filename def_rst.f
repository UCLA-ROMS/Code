...
C$OMP THREADPRIVATE(/priv_scalars/)
     ...
     &  diss,gamma,mass,rhoo
      ...
      integer(kind=4) lenstr, checkdims, my_nf_def_dim
      integer(kind=4) rec, lrst, lvar, timedim,
  ...
  10  if (create_new_file) then
     ...
     &  'time record numbers from initialization for flux analysis')
        ...
        ierr=nf_def_var (ncidrst, vname(1,indxTime)(1:lvar), nf_double,
     &                                         1, timedim, rstTime)
     ...
     &                                   vname(3,indxTime)(1:lvar))
        ...
        ierr=nf_def_var (ncidrst, vname(1,indxZ)(1:lvar), nf_double,
     &                                              3, r2dgrd,rstZ)
     ...
     &                                      vname(3,indxZ)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstZ, '_FillValue', nf_double,
     &                                                 1, spv_set)
        lvar=lenstr(vname(1,indxUb))
        ierr=nf_def_var (ncidrst, vname(1,indxUb)(1:lvar), nf_double,
     &                                           3, u2dgrd, rstUb)
     ...
     &                                     vname(3,indxUb)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstUb, '_FillValue', nf_double,
     &                                                  1, spv_set)
        lvar=lenstr(vname(1,indxVb))
        ierr=nf_def_var (ncidrst, vname(1,indxVb)(1:lvar), nf_double,
     &                                           3, v2dgrd, rstVb)
     ...
     &                                     vname(3,indxVb)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstVb, '_FillValue', nf_double,
     &                                                  1, spv_set)
        ierr=nf_def_var (ncidrst, 'DU_avg2', nf_double, 3, u2dgrd,
     &                                                 rst_DU_avg2)
        ierr=nf_put_att_text (ncidrst, rst_DU_avg2, 'long_name', 38,
     &                    '<<fast-time averaged ubar(:,:,n+1/2)>>')
        ierr=nf_put_att_double (ncidrst, rst_DU_avg2, '_FillValue',
     &                                        nf_double, 1, spv_set)
        ierr=nf_def_var (ncidrst, 'DV_avg2', nf_double, 3, v2dgrd,
     &                                                 rst_DV_avg2)
        ierr=nf_put_att_text (ncidrst, rst_DV_avg2, 'long_name', 38,
     &                    '<<fast-time averaged vbar(:,:,n+1/2)>>')
        ierr=nf_put_att_double (ncidrst, rst_DV_avg2, '_FillValue',
     &                                        nf_double, 1, spv_set)
        lvar=lenstr(vname(1,indxU))
        ierr=nf_def_var (ncidrst, vname(1,indxU)(1:lvar), nf_double,
     &                                           4, u3dgrd, rstU)
     ...
     &                                     vname(3,indxU)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstU, '_FillValue', nf_double,
     &                                                 1, spv_set)
        lvar=lenstr(vname(1,indxV))
        ierr=nf_def_var (ncidrst, vname(1,indxV)(1:lvar), nf_double,
     &                                           4, v3dgrd, rstV)
     ...
     &                                     vname(3,indxV)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstV, '_FillValue', nf_double,
     &                                                 1, spv_set)
        do itrc=1,NT
     ...
     &         vname(3,indxSedFirst+itrc-1)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstTsed(itrc), '_FillValue',
     &         nf_double, 1, spv_set)
        enddo
        lvar=lenstr(vname(1,indxHbls))
        ierr=nf_def_var (ncidrst, vname(1,indxHbls)(1:lvar),
     &                            nf_double, 3, r2dgrd, rstHbls)
     ...
     &                                 vname(3,indxHbls)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstHbls, '_FillValue',
     &                                  nf_double, 1, spv_set)
     ...
     &       vname(3,indxPH_rst)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstPH, '_FillValue', nf_double,
     &                                                 1, spv_set)
        lvar = lenstr(vname(1,indxPCO2_rst))
        ierr=nf_def_var (ncidrst, vname(1,indxPCO2_rst)(1:lvar),
     &       nf_double, 3, r2dgrd, rstPCO2)
     ...
     &       vname(3,indxPAR_rst)(1:lvar))
        ierr=nf_put_att_double (ncidrst, rstPAR, '_FillValue', 
     &                             nf_double,
     &                                                 1, spv_set)
        ierr=nf_enddef(ncidrst)
        if (mynode.eq.0) then
          write(*,'(7x,4A,1x,A,i4)') 'def_rst :: Created new ',
     &          'netCDF file ''', rstname(1:lrst), '''.'
        endif
     ...
     &           'All records beyond this will be overwritten.'
              rec=rec-ierr
     ...
     &       rstPCO2)
        ...
        lvar=lenstr(vname(1,indxPARinc_rst))
        ierr=nf_inq_varid (ncidrst,vname(1,indxPARinc_rst)(1:lvar),
     &       rstPARinc)
        if (ierr. ne. nf_noerr) then
          write(*,1) vname(1,indxPARinc_rst)(1:lvar), rstname(1:lrst)
          goto 99
        endif
        lvar=lenstr(vname(1,indxPAR_rst))
